...
HEADER_BANNER_PATH = _get_asset_path("header_banner.jpg")
FOOTER_LOGO_PATH   = _get_asset_path("footer_logo.png")
...
def _create_college_distribution_chart(college_data):
    ...
    colleges, green_counts, pink_counts, blue_counts = [], [], [], []
    ...
    if colleges:
        ...
        width = 0.25
        ax.bar([i - width for i in x], green_counts, width, label='Green Slips',
               color='#4CAF50', edgecolor='black', linewidth=0.5)
        ax.bar(x, pink_counts, width, label='Pink Slips',
               color='#E91E63', edgecolor='black', linewidth=0.5)
        ax.bar([i + width for i in x], blue_counts, width, label='Blue Slips',
        ...
        ax.text(0.5, 0.5, 'No College Data Available',
...
def _create_slip_distribution_chart(green_count, pink_count, blue_count):
    ...
    if sum(sizes) > 0:
        ...
        ax.text(0.5, 0.5, 'No Slip Data Available',
                ha='center', va='center', fontsize=12, color='gray')
    fig.tight_layout()
    img_buffer = io.BytesIO()
    fig.savefig(img_buffer, format='png', bbox_inches='tight', dpi=90)
...
def generate_overview_report(output_path, records_data, period=None):
    ...
    stats_data  = [_wrap_table_row(stats_data[0], is_header=True)] + \
                  [_wrap_table_row(row) for row in stats_data[1:]]
    ...
    if records_data.get('green'):
        ...
        green_data = [_wrap_table_row(green_data[0], is_header=True)] + \
                     [_wrap_table_row(row) for row in green_data[1:]]
        t = Table(green_data,
                  colWidths=[1.2*inch, 1.5*inch, 0.8*inch, 1.0*inch, 0.9*inch, 0.8*inch],
                  splitByRow=True)
    ...
    if records_data.get('pink'):
        ...
        pink_data = [_wrap_table_row(pink_data[0], is_header=True)] + \
                    [_wrap_table_row(row) for row in pink_data[1:]]
        t = Table(pink_data,
                  colWidths=[1.0*inch, 1.3*inch, 0.8*inch, 1.0*inch, 1.2*inch, 0.9*inch],
                  splitByRow=True)
    ...
    if college_data:
        ...
        college_summary = [_wrap_table_row(college_summary[0], is_header=True)] + \
                          [_wrap_table_row(row) for row in college_summary[1:]]
        college_table = Table(college_summary,
...
def generate_individual_student_report(output_path, student_number,
    ...
    summary_rows  = [_wrap_table_row(summary_rows[0], is_header=True)] + \
                    [_wrap_table_row(row) for row in summary_rows[1:]]
    ...
    if colleges_seen:
        ...
        col_rows  = [_wrap_table_row(col_rows[0], is_header=True)] + \
                    [_wrap_table_row(row) for row in col_rows[1:]]
        col_table = Table(col_rows, colWidths=[0.40 * inch, CONTENT_W - 0.40 * inch])
    ...
    if green_slips:
        ...
        for i, slip in enumerate(green_slips, 1):
            g_rows.append([
                ...
                'Excuse' if slip[5] in (False, 0) else 'Dispensation',
                _safe(slip[7] if len(slip) > 7 else None),
                _safe(slip[9] if len(slip) > 9 else 'Active'),
                _college_label(slip),
            ])
        g_rows  = [_wrap_table_row(g_rows[0], is_header=True)] + \
                  [_wrap_table_row(row) for row in g_rows[1:]]
    ...
    if pink_slips:
        ...
        p_rows  = [_wrap_table_row(p_rows[0], is_header=True)] + \
                  [_wrap_table_row(row) for row in p_rows[1:]]
