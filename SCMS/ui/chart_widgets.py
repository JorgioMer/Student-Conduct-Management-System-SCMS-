# =============================================================================
#  SCMS — Chart Widgets — Reusable chart components with auto-update
#  Uses Agg backend rendered to QPixmap — compatible with Python 3.13 + PyQt5
# =============================================================================
import logging
logger = logging.getLogger('SCMS')

import io
import matplotlib
# Force Agg immediately — safe, non-interactive, no Qt hooking at C level.
# We render figures to PNG bytes and display them in a QLabel/QPixmap instead.
# This is the only backend that works reliably with Python 3.13 + PyQt5.
matplotlib.use("Agg")

from matplotlib.figure import Figure
from matplotlib.backends.backend_agg import FigureCanvasAgg
from matplotlib.ticker import MaxNLocator

from PyQt5.QtWidgets import QWidget, QVBoxLayout, QLabel
from PyQt5.QtGui import QPixmap
from PyQt5.QtCore import Qt

from ui.styles import GREEN_SLIP, PINK_SLIP, BLUE_SLIP, NAVY, OFF_WHITE


class MatplotlibChart(QWidget):
    """
    Base chart widget. Renders a matplotlib Figure to PNG bytes via Agg,
    then displays it in a QLabel — no Qt5Agg / FigureCanvas needed.
    Compatible with Python 3.13 + PyQt5.
    """

    def __init__(self, parent=None):
        super().__init__(parent)
        self.figure = Figure(figsize=(7, 3), dpi=100, facecolor='white')
        self._canvas_agg = FigureCanvasAgg(self.figure)
        self._last_pixmap = None

        self._image_label = QLabel()
        self._image_label.setAlignment(Qt.AlignCenter)
        self._image_label.setMinimumHeight(140)

        layout = QVBoxLayout(self)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.addWidget(self._image_label)
        self.setLayout(layout)

    def clear_figure(self):
        self.figure.clear()

    def _render(self):
        """Render figure to PNG bytes and display as QPixmap in the label."""
        try:
            self._canvas_agg = FigureCanvasAgg(self.figure)
            buf = io.BytesIO()
            self._canvas_agg.print_png(buf)
            buf.seek(0)
            pixmap = QPixmap()
            pixmap.loadFromData(buf.read(), "PNG")
            self._last_pixmap = pixmap
            self._scale_pixmap()
        except Exception as e:
            logger.error(f"Chart render failed: {e}", exc_info=True)
            self._image_label.setText("Chart unavailable")

    def _scale_pixmap(self):
        if self._last_pixmap and not self._last_pixmap.isNull():
            w = self.width()  if self.width()  > 0 else self._last_pixmap.width()
            h = self.height() if self.height() > 0 else self._last_pixmap.height()
            self._image_label.setPixmap(
                self._last_pixmap.scaled(w, h, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            )

    def resizeEvent(self, event):
        super().resizeEvent(event)
        self._scale_pixmap()

    def refresh(self):
        self._render()


class GreenSlipChart(MatplotlibChart):
    def __init__(self, parent=None):
        super().__init__(parent)

    def update_data(self, dispensation_count, excuse_count, active_count, expired_count):
        self.clear_figure()

        ax1 = self.figure.add_subplot(121)
        ax2 = self.figure.add_subplot(122)

        labels1, sizes1, colors1 = [], [], []
        if dispensation_count > 0:
            labels1.append(f'Dispensation ({dispensation_count})')
            sizes1.append(dispensation_count)
            colors1.append('#388E3C')
        if excuse_count > 0:
            labels1.append(f'Excuse ({excuse_count})')
            sizes1.append(excuse_count)
            colors1.append('#66BB6A')

        if sizes1:
            ax1.pie(sizes1, labels=labels1, colors=colors1, autopct='%1.0f%%',
                    startangle=90, textprops={'fontsize': 10, 'weight': 'bold'})
        else:
            ax1.text(0.5, 0.5, 'No data available', ha='center', va='center', fontsize=12)
            ax1.set_xlim(0, 1); ax1.set_ylim(0, 1)
        ax1.set_title('Dispensation vs Excuse', fontsize=11, fontweight='bold', pad=15)

        statuses = ['Active', 'Expired']
        counts   = [active_count, expired_count]
        bars = ax2.bar(statuses, counts, color=['#4CAF50', '#FF9800'],
                       edgecolor='black', linewidth=1.5)
        for bar in bars:
            h = bar.get_height()
            if h > 0:
                ax2.text(bar.get_x() + bar.get_width() / 2., h, f'{int(h)}',
                         ha='center', va='bottom', fontweight='bold', fontsize=10)
        ax2.set_ylabel('Count', fontsize=10, fontweight='bold')
        ax2.set_title('Green Slips by Status', fontsize=11, fontweight='bold', pad=15)
        ax2.set_ylim(0, max(counts) * 1.15 if any(counts) else 1)
        ax2.yaxis.set_major_locator(MaxNLocator(integer=True))
        ax2.grid(axis='y', alpha=0.3, linestyle='--')

        self.figure.tight_layout()
        self._render()


class BlueSlipChart(MatplotlibChart):
    def __init__(self, parent=None):
        super().__init__(parent)

    def update_data(self, violation_types, pending_count, escalated_count, resolved_count):
        self.clear_figure()

        ax1 = self.figure.add_subplot(121)
        ax2 = self.figure.add_subplot(122)

        if violation_types:
            types  = list(violation_types.keys())[:8]
            counts = [violation_types[t] for t in types]
            colors_list = [BLUE_SLIP if i % 2 == 0 else '#64B5F6' for i in range(len(types))]
            bars = ax1.bar(range(len(types)), counts, color=colors_list,
                           edgecolor='black', linewidth=1.5)
            for bar in bars:
                h = bar.get_height()
                if h > 0:
                    ax1.text(bar.get_x() + bar.get_width() / 2., h, f'{int(h)}',
                             ha='center', va='bottom', fontweight='bold', fontsize=9)
            ax1.set_xticks(range(len(types)))
            ax1.set_xticklabels(types, rotation=45, ha='right', fontsize=9)
            ax1.set_ylabel('Count', fontsize=10, fontweight='bold')
            ax1.set_ylim(0, max(counts) * 1.15 if counts else 1)
            ax1.yaxis.set_major_locator(MaxNLocator(integer=True))
            ax1.grid(axis='y', alpha=0.3, linestyle='--')
        else:
            ax1.text(0.5, 0.5, 'No violation data', ha='center', va='center', fontsize=12)
            ax1.set_xlim(0, 1); ax1.set_ylim(0, 1)
        ax1.set_title('Violations by Type', fontsize=11, fontweight='bold', pad=15)

        labels2, sizes2, colors2 = [], [], []
        if pending_count   > 0: labels2.append(f'Pending ({pending_count})');     sizes2.append(pending_count);   colors2.append('#F57F17')
        if escalated_count > 0: labels2.append(f'Escalated ({escalated_count})'); sizes2.append(escalated_count); colors2.append('#D32F2F')
        if resolved_count  > 0: labels2.append(f'Resolved ({resolved_count})');   sizes2.append(resolved_count);  colors2.append('#2E7D32')

        if sizes2:
            ax2.pie(sizes2, labels=labels2, colors=colors2, autopct='%1.0f%%',
                    startangle=90, textprops={'fontsize': 10, 'weight': 'bold'})
        else:
            ax2.text(0.5, 0.5, 'No status data', ha='center', va='center', fontsize=12)
            ax2.set_xlim(0, 1); ax2.set_ylim(0, 1)
        ax2.set_title('Violations by Status', fontsize=11, fontweight='bold', pad=15)

        self.figure.tight_layout()
        self._render()


class PinkSlipChart(MatplotlibChart):
    def __init__(self, parent=None):
        super().__init__(parent)

    def update_data(self, violation_types, year_distribution):
        self.clear_figure()

        ax1 = self.figure.add_subplot(121)
        ax2 = self.figure.add_subplot(122)

        if violation_types:
            types  = list(violation_types.keys())[:8]
            counts = [violation_types[t] for t in types]
            colors_list = [PINK_SLIP if i % 2 == 0 else '#F48FB1' for i in range(len(types))]
            bars = ax1.bar(range(len(types)), counts, color=colors_list,
                           edgecolor='black', linewidth=1.5)
            for bar in bars:
                h = bar.get_height()
                if h > 0:
                    ax1.text(bar.get_x() + bar.get_width() / 2., h, f'{int(h)}',
                             ha='center', va='bottom', fontweight='bold', fontsize=9)
            ax1.set_xticks(range(len(types)))
            ax1.set_xticklabels(types, rotation=45, ha='right', fontsize=9)
            ax1.set_ylabel('Count', fontsize=10, fontweight='bold')
            ax1.set_ylim(0, max(counts) * 1.15 if counts else 1)
            ax1.yaxis.set_major_locator(MaxNLocator(integer=True))
            ax1.grid(axis='y', alpha=0.3, linestyle='--')
        else:
            ax1.text(0.5, 0.5, 'No violation data', ha='center', va='center', fontsize=12)
            ax1.set_xlim(0, 1); ax1.set_ylim(0, 1)
        ax1.set_title('Pink Slips by Violation Type', fontsize=11, fontweight='bold', pad=15)

        if year_distribution:
            years  = list(year_distribution.keys())
            counts = [year_distribution[y] for y in years]
            colors_list = ['#FF6F00', '#FF8A50', '#FFB74D', '#FFCC80', '#FFE0B2'][:len(years)]
            ax2.pie(counts, labels=years, colors=colors_list, autopct='%1.0f%%',
                    startangle=90, textprops={'fontsize': 10, 'weight': 'bold'})
        else:
            ax2.text(0.5, 0.5, 'No grade data', ha='center', va='center', fontsize=12)
            ax2.set_xlim(0, 1); ax2.set_ylim(0, 1)
        ax2.set_title('Pink Slips by Grade Level', fontsize=11, fontweight='bold', pad=15)

        self.figure.tight_layout()
        self._render()


class CombinedAllSlipsChart(MatplotlibChart):
    def __init__(self, parent=None):
        super().__init__(parent)

    def update_data(self, green_count, pink_count, blue_count):
        self.clear_figure()

        ax1 = self.figure.add_subplot(121)
        ax2 = self.figure.add_subplot(122)

        labels, sizes, colors = [], [], []
        if green_count > 0: labels.append(f'Green ({green_count})'); sizes.append(green_count); colors.append(GREEN_SLIP)
        if pink_count  > 0: labels.append(f'Pink ({pink_count})');   sizes.append(pink_count);  colors.append(PINK_SLIP)
        if blue_count  > 0: labels.append(f'Blue ({blue_count})');   sizes.append(blue_count);  colors.append(BLUE_SLIP)

        if sizes:
            ax1.pie(sizes, labels=labels, colors=colors, autopct='%1.0f%%',
                    startangle=90, textprops={'fontsize': 10, 'weight': 'bold'})
        else:
            ax1.text(0.5, 0.5, 'No data available', ha='center', va='center', fontsize=12)
            ax1.set_xlim(0, 1); ax1.set_ylim(0, 1)
        ax1.set_title('All Slip Records by Type', fontsize=11, fontweight='bold', pad=15)

        slip_types = ['Green', 'Pink', 'Blue']
        counts     = [green_count, pink_count, blue_count]
        bar_colors = [GREEN_SLIP, PINK_SLIP, BLUE_SLIP]
        bars = ax2.bar(slip_types, counts, color=bar_colors, edgecolor='black', linewidth=1.5)
        for bar in bars:
            h = bar.get_height()
            if h > 0:
                ax2.text(bar.get_x() + bar.get_width() / 2., h, f'{int(h)}',
                         ha='center', va='bottom', fontweight='bold', fontsize=11)
        ax2.set_ylabel('Count', fontsize=10, fontweight='bold')
        ax2.set_title('Total Monthly Records by Type', fontsize=11, fontweight='bold', pad=15)
        ax2.set_ylim(0, max(counts) * 1.15 if any(counts) else 1)
        ax2.yaxis.set_major_locator(MaxNLocator(integer=True))
        ax2.grid(axis='y', alpha=0.3, linestyle='--')

        self.figure.tight_layout()
        self._render()